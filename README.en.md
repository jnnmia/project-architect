...
| **Cline** | `.clinerules` | Marker-wrapped VS Code Cline extension |
| **Continue** | `.continue/prompts/project-rules.md` | Marker-wrapped Continue rule prompt path |
...
anyway, so it can ride along as a default. The other eight targets must each be confirmed.
...
# Reports one line per supported tool; writes nothing. Exits 0 by default, or 1 if --fail-if-empty is set and no traces exist.
...
# Ensures no unresolved placeholders remain, MUST/RFC 2119 (or Chinese normative) keywords exist, and rationales are provided.
...
# Step 5: Apply for real, once the preview looks right (optionally pass --max-rule-len to control truncation, 0 disables)
python scripts/scaffold_rules.py inject \
  --dir /path/to/project \
  --agents "agents,claude,cursor" \
  --strict

# Step 6: Rollback and clean up injected rules (eject, supports --dry-run)
# Strips lifecycle marker blocks safely; removes pure generated files and cleans empty parent directories.
...
> preview cannot disagree with the real run. To roll back an applied change safely,
> run the `eject` subcommand.
...
| Code | Meaning |
|---|---|
| `0` | Success (including `detect` finding no traces by default) |
| `1` | Rule-level failure: validation errors, no principles extracted under `--strict`, an unrecognised `--agents` key, or `detect --fail-if-empty` finding no tool traces |
...
|---|---|---|
| **v1.2.0** | 2026-09-18 | Adds reverse rollback and cleanup subcommand `eject` (with marker stripping, pure generated file removal, and `--dry-run`); expands agent ecosystem with native Cline (`.clinerules`) and Continue (`.continue/prompts/project-rules.md`) support; adjusts `detect` default exit code to 0 on empty project (adds `--fail-if-empty` for strict CI); validates Chinese normative assertions (必须/严禁/不得/禁止) and rationale terms; adds `--max-rule-len` (default 200, 0 disables). Test suite: 61 → 68 cases. |
