...
| **Cline** | `.clinerules` | 标记包裹，适配 VS Code Cline 扩展 |
| **Continue** | `.continue/prompts/project-rules.md` | 标记包裹，适配 Continue 规则提示词路径 |
...
# 每个支持的工具各输出一行；不写任何文件。默认退出码 0，加 --fail-if-empty 时在无痕迹时退出 1。
...
# 步骤 3：合规检查 (validate)
# 校验占位符是否全替换、是否包含 MUST/必须 强制约束、是否有 Rationale/架构理由 论证。
...
# 步骤 5：预览无误后真正落盘 (可传 --max-rule-len 控制截断，0 为不截断)
python scripts/scaffold_rules.py inject \
  --dir /path/to/project \
  --agents "agents,claude,cursor" \
  --strict

# 步骤 6：逆向清理与回滚 (eject，支持 --dry-run)
# 安全剥离生命周期标记块；若文件无用户手写内容则直接移除文件并清理空父目录。
...
> 需逆向剥离注入的规则时，直接运行 `eject` 子命令即可安全回滚，无需手动编辑。
...
| 码 | 含义 |
|---|---|
| `0` | 成功（含 `detect` 探测为空时的默认返回） |
| `1` | 规则层失败：校验不通过、`--strict` 下未提取到任何原则、`--agents` 中出现无法识别的键、`detect --fail-if-empty` 未发现任何工具痕迹 |
...
| 版本 | 发布日期 | 变更说明 |
|---|---|---|
| **v1.2.0** | 2026-09-18 | 新增逆向回滚与清理子命令 eject（支持精确剥离标记块与 --dry-run）；扩充开源 Agent 生态，原生支持 Cline（.clinerules）与 Continue（.continue/prompts/project-rules.md）；detect 探测在无痕迹时默认退出码优化为 0（新增 --fail-if-empty 满足严格门禁）；validate 全面兼容纯中文项目强断言词（必须/严禁/不得/禁止）与架构理由词；inject 新增 --max-rule-len 弹性参数（默认 200，支持 0 不截断）。测试用例 61 → 68。 |
