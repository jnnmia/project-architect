...
| 结构 | 示例 | 支持情况 |
|---|---|---|
| `## Core Principles` + `### 原则名` | 本文档示例 | 首选 |
| `## 原则名`（整节即一条原则） | `## I. 分层隔离` | 支持 |
| `## Core Principles` 下直接挂 bullets | 无 `###` 子标题 | 支持，整节归入一条「Core Principles」 |

> 若以上三种都解析不到任何原则，`inject` 会写入一段占位摘要而非真实规则。
> 因此建议配合 `--strict` 使用 —— 提取不到原则时直接失败，而不是悄悄写个空壳进去。

**校验器识别的占位符**：仅当方括号标记**含下划线且不少于 6 个字符**（如 `[PROJECT_NAME]`）才算未填充的模板槽位。
`[TODO]`、`[NOTE]` 这类普通排版标记不会误报；但 `[STATUS_OK]` 这种形状确实与占位符同构，仍会被判为未填充，
属于启发式判断的固有边界。
