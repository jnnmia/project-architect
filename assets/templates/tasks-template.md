...
每个任务建议关联到需求编号（如 `[REQ-01]`）或核心规则编号（如 `[RULE-II]`），保持改动目标明确可溯源。
标记说明：`[P]` 代表可并行执行；`[S]` 代表串行依赖前置任务。

| 任务编号 | 关联需求 | 类别 | 描述 | 目标文件 |
|---|---|---|---|---|
| `TASK-001` | `[RULE-I]` | 基础脚手架 | 建立模块目录骨架与类型定义 | `src/models/` |
| `TASK-002` | `[RULE-II]` | 测试先行 | 编写第一阶段失败测试套件 | `tests/test_core.py` |
...
- [ ] `TASK-005` [P] [RULE-III] 依赖检查与无用代码清理，验证内存/启动基线
  - *目标文件*: 全局
  - *验证命令*: `python -m pytest --durations=10`
- [ ] `TASK-006` [P] [RULE-IV] 跨平台路径与安全扫描，更新项目文档
